...
it('should return 200 and list all transactions found by userAuthId', async () => {
  ...
  let res = await request(app)
    .get(`/api/v1/transaction/user?userAuthId=${dummyUserAttrs1.authId}`)
    .expect(200);
  ...
  res = await request(app)
    .get(`/api/v1/transaction/user?userAuthId=${dummyUserAttrs2.authId}`)
    .expect(200);
...
it('should return empty array if userAuthId is not found', async () => {
  let res = await request(app).get(`/api/v1/transaction/user?userAuthId=12313`).expect(200);
